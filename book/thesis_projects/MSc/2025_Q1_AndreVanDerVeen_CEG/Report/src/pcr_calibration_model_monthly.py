...
def _to_naive_timestamp(value):
    """Convert datetime-like input to a timezone-naive pandas.Timestamp."""
...
def aggregate_to_monthly(daily_data, time_coord):
    """Aggregate daily data to monthly means.
    
    Args:
        daily_data: Array of daily values (length = number of days).
        time_coord: xarray time coordinate (daily) for grouping into months.
        
    Returns:
        Tuple of (monthly_data, monthly_time):
        - monthly_data: Array of monthly mean values
        - monthly_time: Corresponding monthly time coordinates
    """
    df = pd.DataFrame({
    ...
    monthly_grouped = df.groupby('year_month')['value'].mean()
    
    # Convert PeriodIndex back to timestamps (end of month)
    monthly_time = monthly_grouped.index.to_timestamp()
    monthly_data = monthly_grouped.values
    
    return monthly_data, monthly_time


def read_grdc_monthly_values(start_date, end_date, station_id, data_dir):
    """Read GRDC monthly discharge values for a station and date range.

    Args:
        start_date: Start date, e.g. "1966-01-01".
        end_date: End date, e.g. "1966-12-31".
        station_id: GRDC station id used in file name.
        data_dir: Directory that contains GRDC monthly text files.
    ...
    # Normalize to naive timestamps to avoid timezone-aware/naive comparisons later.
...
class CalibrationObjective:
    ...
    of station dicts with 'name' and 'coords'. Observation input can be either:
    - 'obs_data' (xarray Dataset with streamflow/time), or
    - monthly-only mode: 'station_id' + 'grdc_data_dir'.

    An optional 'weight' key (default 1.0) controls the contribution of each
    station to the aggregated objective::
    ...
    def __init__(
        ...
        Args:
            stations: List of dicts, each with keys:
                ...
                - 'obs_data' (xarray Dataset) from ewatercycle.observation.grdc, OR
                - monthly mode only: 'station_id' and 'grdc_data_dir'
                - 'weight'   (float, optional) default 1.0
            frequency: 'daily' or 'monthly' - temporal resolution for comparison
            calibration_start: Start date for metric calculation window (optional).
                If None, uses cal_start. Format: pd.Timestamp or string parseable by pd.to_datetime
            calibration_end: End date for metric calculation window (optional).
                If None, uses cal_end. Format: pd.Timestamp or string parseable by pd.to_datetime
        """
        ...
        self.fail_objective_penalty = float(fail_objective_penalty)
        self.write_candidate_rows = bool(write_candidate_rows)

        weight_sum = float(nse_weight) + float(kge_weight)
        if weight_sum <= 0:
            raise ValueError("nse_weight + kge_weight must be > 0")
        self.nse_weight = float(nse_weight) / weight_sum
        self.kge_weight = float(kge_weight) / weight_sum

        self.runs_dir = self.output_dir / runs_subdir
        self.runs_dir.mkdir(parents=True, exist_ok=True)
        
        # Calibration window defaults to full model period if not specified
        self.calibration_start = _to_naive_timestamp(calibration_start) if calibration_start is not None else _to_naive_timestamp(cal_start)
        self.calibration_end = _to_naive_timestamp(calibration_end) if calibration_end is not None else _to_naive_timestamp(cal_end)
        
        if self.frequency not in ["daily", "monthly"]:
            raise ValueError(f"frequency must be 'daily' or 'monthly', got {frequency}")

        # Allow monthly station setup directly from GRDC station file metadata.
        prepared_stations = []
        for station in self.stations:
            ...
            if 'obs_data' not in st:
                if self.frequency == "monthly" and 'station_id' in st and 'grdc_data_dir' in st:
                    st['obs_monthly_df'] = read_grdc_monthly_values(
                        start_date=self.calibration_start,
                        end_date=self.calibration_end,
                        station_id=st['station_id'],
                        data_dir=st['grdc_data_dir'],
                    )
                else:
                    raise ValueError(
                        "Each station needs 'obs_data', or for monthly mode provide "
                        "'station_id' and 'grdc_data_dir'."
                    )

            prepared_stations.append(st)

        self.stations = prepared_stations
    ...
    def __call__(self, params_normalized, iteration=None):
        ...
        try:
            run_dir = self.runs_dir / f"run_{run_tag}"
            ...
            # Model is updated daily over the configured setup period.
            n_days = len(pd.date_range(start=_to_naive_timestamp(self.cal_start), end=_to_naive_timestamp(self.cal_end), freq="D"))
            ...
            for station in self.stations:
                ...
                model_time_daily = pd.date_range(
                    start=_to_naive_timestamp(self.cal_start),
                    periods=len(sim),
                    freq="D",
                )

                if 'obs_data' in station:
                    obs = station['obs_data']['streamflow'].values
                    time_coord = pd.to_datetime(station['obs_data']['time'].values)

                    # Aggregate to monthly if specified
                    if self.frequency == "monthly":
                        sim, monthly_time = aggregate_to_monthly(sim, model_time_daily)
                        obs, _ = aggregate_to_monthly(obs, time_coord)
                        eval_time = monthly_time
                    else:
                        eval_time = time_coord

                    # Slice to calibration window
                    cal_mask = (eval_time >= self.calibration_start) & (eval_time <= self.calibration_end)
                    sim = sim[cal_mask]
                    obs = obs[cal_mask]
                else:
                    # Monthly-only path using station_id + grdc_data_dir.
                    if self.frequency != "monthly":
                        raise ValueError(
                            f"Station '{name}' has no obs_data; this is only supported for monthly frequency."
                        )

                    sim_monthly, sim_monthly_time = aggregate_to_monthly(sim, model_time_daily)
                    sim_df = pd.DataFrame({
                        'date': pd.to_datetime(sim_monthly_time),
                        'sim': sim_monthly,
                    })
                    sim_df['month'] = sim_df['date'].dt.to_period('M')

                    obs_df = station['obs_monthly_df'].copy()
                    obs_df['date'] = pd.to_datetime(obs_df['date'])
                    obs_df['month'] = obs_df['date'].dt.to_period('M')

                    cal_start_m = self.calibration_start.to_period('M')
                    cal_end_m = self.calibration_end.to_period('M')
                    sim_df = sim_df[(sim_df['month'] >= cal_start_m) & (sim_df['month'] <= cal_end_m)]
                    obs_df = obs_df[(obs_df['month'] >= cal_start_m) & (obs_df['month'] <= cal_end_m)]

                    merged = sim_df[['month', 'sim']].merge(
                        obs_df[['month', 'value']],
                        on='month',
                        how='inner',
                    ).sort_values('month')

                    sim = merged['sim'].to_numpy()
                    obs = merged['value'].to_numpy()
                ...
                station_obj = self.nse_weight * (1 - nse) + self.kge_weight * (1 - kge)
                station_metrics[name] = {'nse': nse, 'kge': kge, 'obj': station_obj}
                weighted_obj_sum += weight * station_obj
                print(f"  {name}: NSE={nse:.4f}, KGE={kge:.4f}, obj={station_obj:.4f} (w={weight}) [{self.frequency}]")
            ...
            import traceback
            traceback.print_exc()
            return self.fail_objective_penalty
        finally:
